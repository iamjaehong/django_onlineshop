...
function AjaxCreateOrder(e) {
    ...
    request.fail(function(jqXHR, textStatus) {
        if(jqXHR.status == 404) {
            ...
            alert("AjaxCreateOrder문제가 발생했습니다.\n"+order_create_url+"다시 시도해주세요.");
        }
    });
    return order_id;
}
...
function AjaxStoreTransaction(e, order_id, amount, type) {
    ...
    request.fail(function(jqXHR, textStatus) {
        if(jqXHR.status == 404) {
            ...
            alert("AjaxStoreTransaction문제가 발생했습니다.\n"+order_checkout_url+"다시 시도해주세요.");
        }
    });
    return merchant_id;
}
...
function ImpTransaction(e, order_id, merchant_id, imp_id, amount) {
    ...
    request.fail(function(jqXHR, textStatus) {
        if(jqXHR.status == 404) {
            ...
            alert("ImpTransaction문제가 발생했습니다.\n다시 시도해주세요.");
        }
    });
}